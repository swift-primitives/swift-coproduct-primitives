...
where repeat each Element: Comparison.`Protocol` & ~Copyable & ~Escapable {
    ...
    public static func < (lhs: borrowing Coproduct, rhs: borrowing Coproduct) -> Bool {
        // Pack-position lexicographic compare: first by active position,
        // then by payload at matching positions. The mechanism for
        // retrieving active position is speculative — the semantic
        // contract is "compare positions first, fall through to payload
        // on tie".
        switch (lhs, rhs) {
