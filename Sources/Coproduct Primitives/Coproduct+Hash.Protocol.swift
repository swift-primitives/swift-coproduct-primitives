...
// top-level gate.
    ...
    where repeat each Element: Hash.`Protocol` & ~Copyable & ~Escapable {
        ...
        public borrowing func hash(into hasher: inout Hasher) {
            switch self {
            case .at(let value):
                // Placeholder body. Contract: feed the active position's
                // index into the hasher before the payload; the mechanism
                // for retrieving the active position follows the shipped
                // pack-eliminator syntax. Indicative shipped form:
                //
